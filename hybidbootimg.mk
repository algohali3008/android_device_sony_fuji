...
INITSH := $(LOCAL_PATH)/combinedroot/initd.sh
BOOTREC_DEVICE := $(TARGET_RECOVERY_ROOT_OUT)/etc/bootrec-device
TWRP := $(LOCAL_PATH)/recovery/binary/ramdisk-twrp.cpio
...
$(INSTALLED_BOOTIMAGE_TARGET): $(PRODUCT_OUT)/kernel $(uncompressed_ramdisk) $(recovery_uncompressed_ramdisk) $(INSTALLED_RAMDISK_TARGET) $(INITSH) $(PRODUCT_OUT)/utilities/busybox $(PRODUCT_OUT)/utilities/extract_elf_ramdisk $(MKBOOTIMG) $(MINIGZIP) $(INTERNAL_BOOTIMAGE_FILES)
	...
	$(hide) python $(MKELF) -o $@ $(PRODUCT_OUT)/kernel@0x40208000 $(PRODUCT_OUT)/combinedroot.fs@0x41500000,ramdisk device/sony/fuji/prebuilt/RPM.bin@0x20000,rpm
...
$(INSTALLED_RECOVERYIMAGE_TARGET): $(MKBOOTIMG) \
	...
	$(hide) python $(MKELF) -o $@ $(PRODUCT_OUT)/kernel@0x40208000 $(PRODUCT_OUT)/ramdisk-recovery.img@0x41500000,ramdisk device/sony/fuji/prebuilt/RPM.bin@0x20000,rpm
